...
class UnifiedProvider:
    ...
    def __init__(self, config: Optional[UnifiedProviderConfig] = None):
        ...
        logger.info(f"UnifiedProvider initialized with default: {self.config.default_provider.value}")
    ...
    def _initialize_providers(self) -> None:
        ...
        if self.config.google_api_key or self.config.default_provider == ProviderType.GOOGLE:
            try:
    ...
    def get_available_providers(self) -> List[ProviderInfo]:
        ...
        for ptype, provider in self._providers.items():
            info.append(ProviderInfo(
                ...
                status=ProviderStatus.AVAILABLE if hasattr(provider, '_client') and provider._client else ProviderStatus.ERROR,
                api_key_set=True
            ))
        return info
...
def create_unified_provider(config: Optional[UnifiedProviderConfig] = None) -> UnifiedProvider:
...
logger.info(f"✅ Providers module initialized")