...
AVAILABLE_MODELS: Dict[str, SoliGuardModel] = {}
...
def load_all_models() -> None:
    ...
    try:
        ...
        for name, config in MINIMAX_MODELS.items():
            AVAILABLE_MODELS[name] = SoliGuardModel(
                name=config.name,
    ...
    try:
        ...
        for name, config in GLM_MODELS.items():
            ...
            AVAILABLE_MODELS[full_name] = SoliGuardModel(
                name=config.name,
                model_id=config.model_id,
                provider=config.provider,
                context_window=config.context_window,
    ...
    # Add default models
    AVAILABLE_MODELS["gemini-2.0-flash"] = SoliGuardModel(
        ...
        security_focus=SecurityFocus(critical_hunting=True, code_audit=True, exploit_gen=True, fix_gen=True),
    ...
    AVAILABLE_MODELS["gpt-4o"] = SoliGuardModel(
        name="GPT-4o",
        model_id="gpt-4o",
        provider="openai",
...
def get_model(name: str) -> Optional[SoliGuardModel]:
    """Get model by name"""
...
def get_models_by_severity(severity: str) -> List[SoliGuardModel]:
