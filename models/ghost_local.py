"""
SoliGuard Model - Ghost Local
Production-grade security model for smart contract vulnerability detection

Author: SoliGuard Team
Description: Ghost Local configuration with full security prompt engineering
"""
...
MODEL_ID = "ghost-local"
MODEL_NAME = "Ghost Local"
PROVIDER = "local"
...
SYSTEM_PROMPT = """You are SoliGuard, an expert Web3 smart contract security auditor powered by Ghost Local.
...
1. Analyze contracts for security vulnerabilities
...
```
allocation:
  target_code: 40%      # 51,200 tokens
  vulnerability_db: 25%   # 32,000 tokens  
  analysis_context: 20% # 25,600 tokens
  output_format: 15%    # 19,200 tokens
...
- Local/offline security analysis
...
@dataclass
class GhostLocalModelConfig:
    """Complete Ghost Local model configuration"""
...
def get_config() -> GhostLocalModelConfig:
    """Get model configuration"""
    return GhostLocalModelConfig()
...
logger.info(f"✅ Ghost Local model loaded: {MODEL_ID}")
